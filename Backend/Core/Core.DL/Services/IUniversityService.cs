...
namespace Core.DL.Services
{
    public interface IUniversityService
    {
        ...
        Task<PagedResponse<ReadUniversityDto>> GetAvailableAsync(string email, int page, int pageSize);
        Task<PagedResponse<ReadUniversityDto>> GetMyUniversitiesAsync(string email, int page, int pageSize);
        Task<ResponseWithValue<ReadUniversityDto>> GetAsync(string name, string userEmail);
